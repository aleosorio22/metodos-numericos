...
const Inversa = () => {
  ...
  const calcularInversa = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${API_URL}/inversa`, {
        A,
        b
      });
      ...
      if (valoresArray.length > 0) {
        return (
          <div className="space-y-4">
            <p className="font-semibold">Solución del sistema usando matriz inversa:</p>
            ...
            <div className="mt-4 p-3 bg-gray-50 rounded-lg">
              ...
              <MathJax>
                {`\\[X = A^{-1}\\cdot b = ${renderVectorLatex(JSON.stringify(valoresArray))}\\]`}
              </MathJax>
            </div>
          </div>
        );
  ...
  return (
    <CalculatorLayout
      title="Método de la Inversa"
      description="Resuelve sistemas de ecuaciones lineales utilizando la matriz inversa donde X = A⁻¹·b."
      onCalculate={calcularInversa}
    ...
    >
      ...
      <div className="mt-4 p-3 bg-gray-50 rounded-lg">
        <p className="text-sm text-gray-600 mb-1">Solución utilizando la inversa:</p>
        <MathJax>
          {`\\[X = A^{-1} \\cdot b\\]`}
        </MathJax>
      </div>
