...
const Cramer = () => {
  ...
  const calcularCramer = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${API_URL}/cramer`, {
        A,
        b
      });
      ...
      if (valoresArray.length > 0) {
        return (
          <div className="space-y-4">
            <p className="font-semibold">Solución por el método de Cramer:</p>
  ...
  return (
    <CalculatorLayout
      ...
      description="Resuelve sistemas de ecuaciones lineales utilizando determinantes para calcular cada variable del sistema."
      onCalculate={calcularCramer}
    ...
    >
      ...
      <div className="mt-4 p-3 bg-gray-50 rounded-lg">
        <p className="text-sm text-gray-600 mb-1">Fórmula de Cramer:</p>
        <MathJax>
          {`\\[x_i = \\frac{|A_i|}{|A|} \\text{ donde } A_i \\text{ es la matriz A con la columna } i \\text{ reemplazada por el vector } b\\]`}
        </MathJax>
      </div>
    </CalculatorLayout>
  )
