...
const RaicesMultiples = () => {
  ...
  const calcularRaicesMultiples = async () => {
    setLoading(true)
    setError(null)
    
    try {
      const response = await axios.post(`${API_URL}/raices-multiples`, {
        f: funcion,
        x0: Number(x0),
  ...
  return (
    <CalculatorLayout
      ...
      description="Encuentra raíces múltiples (con multiplicidad > 1) de ecuaciones no lineales utilizando una modificación del método de Newton-Raphson."
      onCalculate={calcularRaicesMultiples}
