...
const SecanteModificada = () => {
  ...
  const calcularSecanteModificada = async () => {
    setLoading(true)
    setError(null)
    
    try {
      const response = await axios.post(`${API_URL}/secante-modificada`, {
        f: funcion,
        x0: Number(x0),
  ...
  return (
    <CalculatorLayout
      title="Método de la Secante Modificada"
      description="Encuentra raíces de ecuaciones no lineales utilizando una aproximación de la derivada mediante diferencias finitas, combinando características de Newton-Raphson y el método de la secante."
      onCalculate={calcularSecanteModificada}
...
export default SecanteModificada
