...
const Secante = () => {
  ...
  const calcularSecante = async () => {
    setLoading(true)
    setError(null)
    
    try {
      const response = await axios.post(`${API_URL}/secante`, {
        f: funcion,
        x0: Number(x0),
        x1: Number(x1),
  ...
  return (
    <CalculatorLayout
      ...
      description="Encuentra raíces de ecuaciones no lineales utilizando dos puntos iniciales para aproximar la derivada mediante diferencias finitas."
      onCalculate={calcularSecante}
